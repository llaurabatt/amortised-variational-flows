...
def log_lik(mask_Z:bool, mask_Y:bool, Z: int, Y: int, N: int, T: int, phi: float, theta: Array) -> float:
  ...
  log_prob_z_y = jnp.stack(
      [log_prob_z_given_phi(Z)*mask_Z,
       log_prob_y_given_phi_theta(Y)*mask_Y], axis=-1)
...
def log_lik_vectorised(
  ...
  log_lik_vector_fun0 = jax.vmap(log_lik, in_axes=[0, 0, 0, 0, 0, 0, 0, None]) #? (mask_Z, mask_Y, Z, Y, N, T, phi, theta) vectorized along obs i.e. 13 countries (why 0 on phi?)
  # Vectorise along s parameter samples observations
  log_lik_vector_fun = jax.vmap(
      log_lik_vector_fun0, in_axes=[None, None, None, None, None, None, 0, 0]) #(mask_Z, mask_Y, Z, Y, N, T, phi, theta: Array) vectorized along n_samples 

  return log_lik_vector_fun(mask_Z, mask_Y, Z, Y, N, T, phi, theta)
...
def log_prob_joint(
  ...
  loglik = log_lik_vectorised(mask_Z, mask_Y, batch['Z'], batch['Y'], batch['N'], batch['T'],
  ...
  if is_smi:
    loglik = loglik * jnp.broadcast_to(hp[:2], (1, 1, num_modules))
  ...
  log_prob_phi = distrax.Independent(
      tfd.Beta(
        concentration1=jnp.ones(phi_dim)*hp[2],
          concentration0=jnp.ones(phi_dim)*hp[3],
      ),
...
# Pointwise log-likelihood
def log_lik_y(mask_Y: bool, Y: int, T: int, phi: float, theta: Array) -> float:
  """Observational model for the Epidemiology data.
  ...
  Returns:
    Array with the log-likehood for the Poisson module.
  """
  ...
  return log_prob_y_given_phi_theta(Y)*mask_Y


def log_lik_y_vectorised(
    mask_Y: Array,
    Y: Array,
    T: Array,
    phi: Array,
  ...
  Output:
    Array of shape (s, n_y) with each entry
    corresponding to the pointwise log-likelihood of the y-module.
  """

  # Vectorise along n data observations
  log_lik_vector_fun0 = jax.vmap(log_lik_y, in_axes=[0, 0, 0, 0, None]) #? (mask_Y, Y, T, phi, theta) vectorized along obs i.e. 13 countries 
  # Vectorise along s parameter samples observations
  log_lik_vector_fun = jax.vmap(
      log_lik_vector_fun0, in_axes=[None, None, None, 0, 0]) #(mask_Y, Y, T, phi, theta: Array) vectorized along n_samples 

  return log_lik_vector_fun(mask_Y, Y, T, phi, theta)

# Pointwise log-likelihood
def log_lik_z(mask_Z: bool, Z: int, N: int, phi: float) -> float:
  """Observational model for the Epidemiology data.
  ...
  Returns:
    Array with the log-likehood for the Binomial module.
  """

  ## Disease model
  log_prob_z_given_phi = tfd.Binomial(total_count=N, probs=phi).log_prob

  return log_prob_z_given_phi(Z)*mask_Z


def log_lik_z_vectorised(
    mask_Z: Array,
    Z: Array,
    N: Array,
    phi: Array,
  ...
  Output:
    Array of shape (s, n_z) with each entry
    corresponding to the pointwise log-likelihood of the z-module.
  """

  # Vectorise along n data observations
  log_lik_vector_fun0 = jax.vmap(log_lik_z, in_axes=[0, 0, 0, 0]) #? (mask_Z, Z, N, phi) vectorized along obs i.e. 13 countries (why 0 on phi?)
  # Vectorise along s parameter samples observations
  log_lik_vector_fun = jax.vmap(
      log_lik_vector_fun0, in_axes=[None, None, None, 0]) #(mask_Z, Z, N, phi) vectorized along n_samples 

  return log_lik_vector_fun(mask_Z, Z, N, phi)
