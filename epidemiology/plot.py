...
def plot_phi(
  ...
  handles = [
      plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=s.get_facecolor()[0], 
                markersize=10, label=f'{i+1}', alpha=1) for i, s in enumerate(scatters)
  ]

  leg = ax.legend(handles=handles, title='Group', loc='lower center', ncol=4, fontsize='x-small')
...
def plot_theta(
  ...
  sns.kdeplot(
      ...
      ax=g.ax_marg_x,
      # shade=True, # why is this giving error??
      color=pars['colour'],
      legend=False)
  sns.kdeplot(
...
def posterior_samples(
  ...
  plot_name = plot_name + ("" if (eta is None) else f"_eta_{eta:.2f}")+ ("" if (priorhps is None) else priorhps)
  fig, _ = plot_phi(
