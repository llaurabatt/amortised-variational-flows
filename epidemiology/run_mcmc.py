...
#   posterior_sample_dict_init['theta'] = jnp.array([[-1.5, 20]])
  ...
  ### Sample First Stage ###
  samples_path = workdir + f"/mcmc_eta_{config.smi_eta:.2f}_c1_{config.c1:.2f}_c2_{config.c2:.2f}.sav"  
  if os.path.exists(samples_path):
    logging.info("\t Loading final samples")
    with open(samples_path, 'rb') as f:
        posterior_sample_dict = pickle.load(f)
  ...
  plot.posterior_samples(
      ...
      priorhps=f'_c1_{config.c1:.2f}_c2_{config.c2:.2f}',
      workdir_png=workdir,
  )
