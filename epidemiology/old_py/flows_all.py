...
import modularbayes
from modularbayes._src.bijectors.conditional_masked_coupling_extra_all import AllConditionalMaskedCoupling
...
def meta_nsf_phi(
    ...
    hidden_sizes_conditioner_prior: Sequence[int],
    num_bins: int,
    range_min: float = 0.,
    range_max: float = 1.,
    **_,
  ...
  for _ in range(num_layers):
    layer = AllConditionalMaskedCoupling(
        mask=mask,
        bijector=bijector_fn,
        ...
        conditioner_prior=CouplingConditioner(
        output_dim=math.prod(event_shape),
        hidden_sizes=hidden_sizes_conditioner_prior,
        num_bijector_params=num_bijector_params,
        name='conditioner_betahp',
        ),
        conditioner=CouplingConditioner(
            output_dim=math.prod(event_shape),
