...
def log_lik_vectorised(
  ...
  log_lik_vector_fun0 = jax.vmap(log_lik, in_axes=[0, 0, 0, 0, 0, None]) #? (Z, Y, N, T, phi, theta) vectorized along obs i.e. 13 countries (why 0 on phi?)
  # Vectorise along s parameter samples observations
  log_lik_vector_fun = jax.vmap(
      log_lik_vector_fun0, in_axes=[None, None, None, None, 0, 0]) #(Z, Y, N, T, phi, theta: Array) vectorized along n_samples 
...
def log_prob_joint(
  ...
  log_prob_phi = distrax.Independent(
      tfd.Beta(
        concentration1=jnp.ones(phi_dim)*betahp[0],
          concentration0=jnp.ones(phi_dim)*betahp[1],
      ),
...
# Pointwise log-likelihood
def log_lik_y(Y: int, T: int, phi: float, theta: Array) -> float:
  """Observational model for the Epidemiology data.
  ...
  return log_prob_y_given_phi_theta(Y)
...
def log_lik_y_vectorised(
  ...
  # Vectorise along n data observations
  log_lik_vector_fun0 = jax.vmap(log_lik_y, in_axes=[0, 0, 0, None]) #? (Y, T, phi, theta) vectorized along obs i.e. 13 countries 
  # Vectorise along s parameter samples observations
  log_lik_vector_fun = jax.vmap(
      log_lik_vector_fun0, in_axes=[None, None, 0, 0]) #(Z, Y, N, T, phi, theta: Array) vectorized along n_samples 

  return log_lik_vector_fun(Y, T, phi, theta)

# Pointwise log-likelihood
def log_lik_z(Z: int, N: int, phi: float) -> float:
  """Observational model for the Epidemiology data.
...
def log_lik_z_vectorised(
  ...
  # Vectorise along n data observations
  log_lik_vector_fun0 = jax.vmap(log_lik_z, in_axes=[0, 0, 0]) #? (Z, N, phi) vectorized along obs i.e. 13 countries (why 0 on phi?)
  # Vectorise along s parameter samples observations
  log_lik_vector_fun = jax.vmap(
      log_lik_vector_fun0, in_axes=[None, None, 0]) #(Z, N, phi) vectorized along n_samples 

  return log_lik_vector_fun(Z, N, phi)
