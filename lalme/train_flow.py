...
def get_inducing_points(dataset: Batch, config: ConfigDict):
    ...
    # Add jitter
    dataset[
        'cov_inducing'] = dataset['cov_inducing'] + config.gp_jitter * jnp.eye(
    ...
    dataset['cov_inducing_inv'] = jnp.matmul(cov_inducing_chol_inv.T,
                                             cov_inducing_chol_inv,
                                             precision='highest')
    # jnp.matmul(
...
def q_distr_loc_floating(
  ...
  """
    ...
    # Log_probabilities of the sample
    q_distr_out['loc_floating_log_prob'] = locations_log_prob
...
def sample_all_flows(
    ...
    if include_random_anchor:
        ...
        q_distr_out[
            'loc_random_anchor_log_prob'] = q_distr_out_loc_random_anchor[
...
def elbo_fn(
    ...
    if is_smi:
        ...
        for key in shared_params_names:
            posterior_sample_dict_stg1[key] = q_distr_out['posterior_sample'][
                key]
        for key in refit_params_names:
            posterior_sample_dict_stg1[key] = q_distr_out['posterior_sample'][
                key + '_aux']
        elbo_stg1 = log_prob_fun.log_prob_joint(
            batch=batch,
    ...
    for key in shared_params_names:
        if is_smi:
            ...
            posterior_sample_dict_stg2[key] = q_distr_out['posterior_sample'][
                key]
    for key in refit_params_names:
    ...
    if is_smi:
        ...
        log_q_stg2 = (q_distr_out['global_params_log_prob'] +
                      q_distr_out['loc_floating_log_prob'])
    ...
    if include_random_anchor:
        ...
        for key in grad_params_names:
            posterior_sample_dict_stg3[key] = q_distr_out['posterior_sample'][
                key]
        elbo_stg3 = log_prob_fun.log_prob_joint(
            batch=batch,
...
def train_and_evaluate(config: ConfigDict, workdir: str) -> None:
  ...
  """
    ...
    config.num_inducing_points = math.prod(
        config.flow_kwargs.inducing_grid_shape)

    # For training, we need a Dictionary compatible with jit
    # we remove string vector
    train_ds = {
        k: v
        for k, v in dataset.items() if k not in ['items', 'forms']
    }
    ...
    if config.include_random_anchor:
        ...
        state_list.append(
            utils.initial_state_ckpt(
                ...
                forward_fn_kwargs={
                    ...
                    'flow_kwargs': config.flow_kwargs,
                    'global_params_base_sample':
                    global_params_base_sample_init,
                },
                prng_key=next(prng_seq),
                optimizer=make_optimizer(**config.optim_kwargs),
            ))
        ...
        tabulate_fn_ = hk.experimental.tabulate(
            f=lambda state, prng_key: hk.transform(q_distr_loc_random_anchor).
            apply(
                state.params,
                prng_key,
                flow_name=config.flow_name,
    ...
    while state_list[0].step < config.training_steps:
        ...
        if (state_list[0].step
                == 0) or (state_list[0].step % config.log_img_steps == 0):
        ...
        if (smi_eta is not None) and (state_list[0].step < int(
                config.random_eta_steps)):
        ...
        if state_list[0].step == 1:
            logging.info("STEP: %5d; training loss: %.3f",
                         state_list[0].step - 1, metrics["train_loss"])
        ...
        if (state_list[0].step > config.random_eta_steps) and (
            ...
            if config.include_random_anchor:
                metrics['elbo_stage_3'] = jnp.nanmean(
                    elbo_dict_eval['stage_3'])
            ...
            metrics_to_log = [
                'elbo_stage_1', 'elbo_stage_2', 'distance_floating'
            ]
            if config.include_random_anchor:
