...
from modularbayes.typing import Any, Dict, Optional
...
def load_lalme(dataset_id: str = 'coarsen_8_items') -> Dict[str, Any]:
    ...
    files = [
        f'{dataset_id}_loc.csv', f'{dataset_id}_m.csv',
        f'{dataset_id}_items.txt', f'{dataset_id}_forms.txt',
        'floating_profiles_id.txt'
    ]
    ...
    data['y'] = np.split(y_concat_df.to_numpy(),
                         np.cumsum(data['num_forms_tuple'])[:-1],
                         axis=0)
...
def process_lalme(
    ...
    if (num_profiles_anchor_keep < num_profiles_anchor) or (
        ...
        data[
            'num_profiles'] = num_profiles_anchor_keep + num_profiles_floating_keep
    ...
    num_items_keep = (int(num_items_keep)
                      if num_items_keep is not None else num_items)
    ...
    if num_items_keep < num_items:
        ...
        data['num_forms_tuple'] = tuple(data['num_forms_tuple'][i]
                                        for i in index_items_keep)
