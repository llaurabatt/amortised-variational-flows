...
def vmp_map(eta, vmp_map_name, vmp_map_kwargs, params_flow_init):
    return getattr(metaposterior,
                   vmp_map_name)(**vmp_map_kwargs,
                                 params_flow_init=params_flow_init)(eta)
...
def loss(
    ...
    params_flow_vmap_list = [
        ...
        ) for params, params_flow_init in zip(params_tuple,
    ...
    num_profiles_total = (flow_kwargs.num_profiles_anchor +
                          flow_kwargs.num_profiles_floating)
        ...
        for i in range(eta_grid_len):
            ...
            params_flow_eta_grid_eval = [
                vmp_map.apply(
                    ...
                    vmp_map_kwargs=config.vmp_map_kwargs,
                    params_flow_init=params_flow_init,
                ) for state, params_flow_init in zip(state_list,
                                                     params_flow_init_list)
            ]
            ...
            error_loc_dict_i = jax.vmap(compute_error_locations,
                                        in_axes=[0, None])(
  ...
  """
    ...
    if config.include_random_anchor:
        if state_flow_init_path_list[2] == '':
            ...
            state_flow_init = utils.load_ckpt(
                path=state_flow_init_path_list[2])
            params_flow_init_list.append(state_flow_init.params)
            del state_flow_init
    ...
    if config.vmp_map_name in ['VmpGP', 'VmpCubicSpline']:
        ...
        config.vmp_map_kwargs['eta_knots'] = scipy.stats.beta.ppf(
            jnp.linspace(0, 1.,
                         config.vmp_map_kwargs.num_knots).reshape(-1, 1),
    ...
    while state_list[0].step < config.training_steps:
        ...
        if (config.log_img_steps is not None) and (
            (state_list[0].step in [0, 1]) or
