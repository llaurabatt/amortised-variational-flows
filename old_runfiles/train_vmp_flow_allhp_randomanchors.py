...
import log_prob_fun_allhp_randomanchors
from log_prob_fun_allhp_randomanchors import ModelParamsGlobal, ModelParamsLocations, PriorHparams, sample_priorhparams_values
import flows
import plot
from train_flow_allhp_randomanchors import (load_data, make_optimizer, get_inducing_points,
  ...
  for cond_val_, prior_hparams_ in zip(cond_values_chunked_,prior_hparams_chunked_):
    ...
    if include_gamma:
      ...
      gamma_sample_, _, _ = jax.vmap(
          lambda key_, global_, locations_: log_prob_fun_allhp_randomanchors.
...
#####################################################################################
  ...
  hp_star_default_tuple = tuple([float(i) for i in hp_star_default])
  ...
  mse_jit = lambda hp_params, batch, prng_key, state_list_vmp, hp_optim_mask_indices, hp_fixed_values: mse_fixedhp(
      ...
      profile_is_anchor=profile_is_anchor,
      num_profiles_split=num_profiles_split,
  )['mean_dist_random_anchor']

  mse_jit = jax.jit(mse_jit, static_argnames=('hp_optim_mask_indices')) #, 'hp_fixed_values'))
