...
declare -A all_etas
all_etas["3ELBO"]='0.420' #'0.001 0.05 0.250 0.420 0.500 0.750 1.000' 
all_etas["50ELBO"]='0.001 0.05 0.250 0.500 0.610 0.750 1.000'

for n_val in "${N_VAL_SAMPLES[@]}"; do
    for n_elbo in "${N_ELBO_SAMPLES[@]}"; do

        WORK_DIR=$HOME/mount/vmp-output-time/lp-all/$n_elbo/$n_val

        # Create output directory if it does not exist
        mkdir -p $WORK_DIR

        # # VMP and hyperpameter optimisation
        python3 $REPO_DIR/main.py --config $REPO_DIR/configs/all_items_flow_nsf_vmp_flow_TIME_${n_elbo}_${n_val}.py \
                                  --workdir $WORK_DIR \
        ...
        for eta in "${eta_array[@]}"; do
          ...
          python3 $REPO_DIR/main.py --config $REPO_DIR/configs/all_items_flow_nsf_vp_flow_TIME_${n_elbo}_${n_val}.py \
