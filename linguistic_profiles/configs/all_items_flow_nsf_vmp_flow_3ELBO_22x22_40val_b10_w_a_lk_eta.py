...
def get_config():
  ...
  config.cond_hparams_names = ['w_prior_scale', 'a_prior_scale', 'kernel_length_scale', 'eta']
  ...
  config.tune_vmp_hparams = ('w_prior_scale', 'a_prior_scale', 'kernel_length_scale', 'eta')
  ...
  config.flow_kwargs.inducing_grid_shape = (22, 22)
  ...
  config.prior_hparams_hparams.kernel_sampling_lengthscale_alpha = 0.1
  ...
  config.kernel_kwargs.amplitude = 1.0
  config.kernel_kwargs.length_scale = 0.3
  config.gp_jitter = 1e-2
  ...
  # etas at which save_samples dumps posterior netcdfs (lalme_az_eta_*.nc).
  # 0.435 = this run's PMSE-tuned eta_hat; endpoints for Bayes/Cut comparison.
  # NOTE: the dump conditions on PriorHparams DEFAULTS -> when dumping, override
  # --config.kernel_kwargs.length_scale=0.1 so ell_k matches the tuned phi_hat
  # (config default 0.3 is the TRAINING default, do not change it here).
  config.eta_save = [0.001, 0.435, 1.0]
...
#  4682 4685 7550 7591 7592 7593 7600 7610 7980]
  ...
  # eta_hat=0.435 from PMSE tuning 2026-08-06 (RPMSE 0.2772; 11x11 optimum was 0.315).
  config.eta_plot = [0.001, 0.435, 1.0]
  ...
  # PriorHparams field order: (w, a, mu_conc, mu_rate, zeta_a, zeta_b,
  # kernel_AMPLITUDE, kernel_LENGTH_scale) — eta is NOT here, it lives in eta_plot.
  # PLACEHOLDER (11x11 optimum) — update sigma_w/sigma_a/ell_k after this run's
  # PMSE tuning. NB: 22x22 grid + lengthscale sampling extended down to 0.1, so
  # ell_k may tune BELOW the old 0.2 floor here.
  config.prior_hparams_plot_optim = [5., 10.,  1., 0.5, 1., 1., 1.0, 0.1]
